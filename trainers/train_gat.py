...
from models.gat import GATGraphDTA
...
if torch.cuda.is_available():
    ...
    print("To use GPU, install CUDA-enabled PyTorch. Python 3.14 may not have CUDA builds yet.")
...
def train_one_epoch(loader, gat_model, prot_encoder, optimizer, device) -> float:
    """ Trains the GAT model for one epoch.
    Args:
        loader (DataLoader): DataLoader for the training data.
        gat_model (GATGraphDTA): The GAT model to train.
    ...
    for graph_batch, prot_seqs, y in loader:
        ...
        pred = gat_model(graph_batch, prot_vec)
...
@torch.no_grad()
def evaluate(loader, gat_model, prot_encoder, device):
    """ Evaluates the GAT model on the validation set.
    Args:
        loader (DataLoader): DataLoader for the validation data.
        gat_model (GATGraphDTA): The GAT model to evaluate.
        prot_encoder (ProteinEncoder): The protein encoder model.
...
def main():
    ...
    gatModel = GATGraphDTA().to(device)

    gb, ps, yb = next(iter(train_loader))
    pb = torch.stack([tokenizer(s) for s in ps]).to(device)
    proteinEncoder.eval(); gatModel.eval()
    with torch.no_grad():
        pv = proteinEncoder(pb)                 # [B, 128]
        pr = gatModel(gb.to(device), pv)        # [B, 1]
    print("SMOKE:", pb.shape, pv.shape, pr.shape)

    optimizer = torch.optim.Adam(list(gatModel.parameters()) + list(proteinEncoder.parameters()), lr=LR)
    os.makedirs("checkpoints", exist_ok=True)

    best_val = float('inf')

    for epoch in range(EPOCHS):
        train_loss_mse = train_one_epoch(train_loader, gatModel, proteinEncoder, optimizer, device)
        validation_mse, validation_ci = evaluate(val_loader, gatModel, proteinEncoder, device)
        ...
        if validation_mse < best_val:
            ...
            torch.save({
                'epoch': epoch + 1,
                'model_state_dict': gatModel.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
                'protein_encoder_state_dict': proteinEncoder.state_dict(),
            }, "checkpoints/best_model_gat.pth")
    
    ckpt = torch.load("checkpoints/best_model_gat.pth", map_location=device, weights_only=False)
    gatModel.load_state_dict(ckpt['model_state_dict'])
    proteinEncoder.load_state_dict(ckpt['protein_encoder_state_dict'])
    test_mse, test_ci = evaluate(test_loader, gatModel, proteinEncoder, device)
